...
export class LSTMMultivariateTimeSeries extends LSTMTimeSeries {
    ...
    static createDataset(dataset = [], look_back = 1) {
        ...
        const x = LSTMMultivariateTimeSeries.drop(droppedColumns, original_dropped_c_columns);
        //@ts-ignore
        return [x, y, features,];
    }
    /**
  ...
  const newdata = drop(data,dropColumns); //=> [
    ...
    * @param {Array<number>} columns - array of column indexes
    ...
    static seriesToSupervised(data, n_in = 1, n_out = 1) {
        if (n_out !== 1)
            throw new RangeError('Only 1 future iteration supported');
        if (data && Array.isArray(data) && Array.isArray(data[0]) && data[0].length < 2)
            throw new RangeError('Must include at least two columns, the first column must be the dependent variable, followed by independent variables');
