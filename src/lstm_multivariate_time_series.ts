...
export class LSTMMultivariateTimeSeries extends LSTMTimeSeries {
  ...
  static createDataset(this:TensorScriptModelContext, dataset:NestedArray<Array<number>> = [], look_back:number = 1):NestedArray<number> { 
    ...
    //@ts-ignore
    const droppedColumns:any = LSTMMultivariateTimeSeries.drop(series, dropped);
...
const newdata = drop(data,dropColumns); //=> [ 
  ...
  * @param {Array<number>} columns - array of column indexes
   ...
   */
  static seriesToSupervised(data: number[], n_in:number = 1, n_out:number = 1) {
    ...
    for (let x in data) {
      ...
      x = Number(parseInt(x));
      //@ts-ignore
      let maxIndex:number = x + n_in + n_out;
...
const dropped = getDropableColumns(8, n_in, n_out); //=> [ 10, 11, 12, 13, 14, 15, 16, 17 ]
   ...
   */
  static getDropableColumns(features: number, n_in: number, n_out: number) {
    if (n_out !== 1) throw new RangeError('Only 1 future iteration supported');
   ...
   */
  static getTimeseriesShape(this:TensorScriptModelContext, x_timeseries: NestedArray<any> | undefined) {
   ...
   */
  generateLayers(x_matrix:Matrix, y_matrix:Matrix, layers: TensorScriptSavedLayers) {
   ...
   * @param y_test 
   */
  async train(this:TensorScriptLSTMModelContext,x_timeseries: any, y_timeseries: any, layers:TensorScriptLayers, x_test:Matrix, y_test:Matrix) {
    let xShape;
    let yShape;
    let x_matrix;
    let y_matrix;
 ...
 */
/* istanbul ignore next */
export function pivotVector(vectors:Matrix=[]):Matrix {
  /* istanbul ignore next */
  return vectors.reduce((result:Matrix, val, index/*, arr*/) => {
