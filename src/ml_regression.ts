 ...
 * @class MachineLearningLinearRegression
 * @implements {MachineLearningModelInterface}
 */
export class MachineLearningLinearRegression extends MachineLearningModelInterface {
  /**
  ...
  constructor(options:TensorScriptOptions = {}, properties?:TensorScriptProperties) {
    ...
    if(options?.fit?.callbacks) config.fit.callbacks =[        
      this.tf.callbacks.earlyStopping({ monitor: 'mse', patience: 30 })
    ].concat(createModelFitCallback(options?.fit?.callbacks));

    this.type = 'MachineLearningLinearRegression';
    this.model = new this.scikit.LinearRegression({
